...
const BottomInfo = () => {
  ...
  const longShortHistory = tableData?.map((item: ILongShortData) => {
    const isLong =
...
const titleHistoryRow = [
  ...
  },
  {
    title: "Leverage",
    field: (data: any) => {
      return <div>{`${data.marginLevel.toFixed(2)} %`}</div>;
    },
  },
  {
    title: "Collateral",
    field: (data: any) => {
      return (
        <div className="whitespace-nowrap">{`${data.baseTokenAmount.toFixed(
          4,
        )} ${data.direction.toLowerCase() === "long" ? "ETH" : "DAI"}`}</div>
      );
    },
  },
  // { title: "Collateral", field: "baseTokenAmount" },
  {
    ...
    field: (data: any) => {
      return (
        <div className="whitespace-nowrap">{`${data.quoteTokenAmount.toFixed(
          4,
        )} ${data.direction.toLowerCase() === "long" ? "DAI" : "ETH"}`}</div>
      );
    },
  },
  ...
  { title: "Liquidation Price", field: "liquidPrice" },
  {
    title: "Margin Ratio",
    field: (data: any) => {
      return <div>{`${data.marginRatio.toFixed(2)}`}</div>;
    },
  },
  {
    title: "PnL (%)",
