...
const RightContent = () => {
  ...
  const fetchRewardData = useCallback(
    async (userInvestYield: Array<any>) => {
      const data = await Promise.all(
        userInvestYield.map(async (yeild) => {
          const price = await fetchPriceData(yeild.asset);
          return {
            title: rewardTokens.get(yeild.asset).symbol,
            value: new Decimal(yeild.amount._hex).div(
              new Decimal(10).pow(rewardTokens.get(yeild.asset).decimals),
            ),
            netValue: new Decimal(price || 0).mul(
              new Decimal(yeild.amount._hex).div(
                new Decimal(10).pow(rewardTokens.get(yeild.asset).decimals),
              ),
            ),
          };
